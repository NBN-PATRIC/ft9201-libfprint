...
main (int argc, char **argv)
{
  ...
  /* Carrega tudo primeiro para poder escolher a ordem de fusao. */
  static unsigned char all[64][IMG_SZ];
  int ok[64] = {0}, nf = 0;
  for (int i = 0; i < n; i++)
    if (load_pgm (paths[i], all[nf]) == 0) { ok[nf] = 1; nf++; }

  mosaic_reset ();
  int used[64] = {0};
  mosaic_paste (all[0], PAD, PAD); used[0] = 1;
  printf ("  ancora: frame 1\n");

  /* Fusao gulosa: a cada rodada agrega o frame que melhor casa com o
   * composto atual, em vez de seguir a ordem de captura. */
  for (;;)
    {
      ...
      for (int k = 0; k < nf; k++)
        {
          if (used[k]) continue;
          double lb = -2.0; int ly = PAD, lx = PAD, found = 0;
          for (int dy = PAD - ALIGN_RANGE; dy <= PAD + ALIGN_RANGE; dy += ALIGN_COARSE)
            for (int dx = PAD - ALIGN_RANGE; dx <= PAD + ALIGN_RANGE; dx += ALIGN_COARSE)
              { double c = mosaic_corr (all[k], dy, dx); if (c > lb) { lb = c; ly = dy; lx = dx; found = 1; } }
          if (!found) continue;
          for (int dy = ly - ALIGN_COARSE; dy <= ly + ALIGN_COARSE; dy++)
            for (int dx = lx - ALIGN_COARSE; dx <= lx + ALIGN_COARSE; dx++)
              { double c = mosaic_corr (all[k], dy, dx); if (c > lb) { lb = c; ly = dy; lx = dx; } }
          if (lb > best) { best = lb; bk = k; by = ly; bx = lx; }
        }
      if (bk < 0 || best < ALIGN_MIN_CORR)
        { printf ("    (restantes abaixo de %.2f, parando)\n", ALIGN_MIN_CORR); break; }
      printf ("    + frame %d: corr=%.2f offset=(%+d,%+d)\n", bk + 1, best, by - PAD, bx - PAD);
      mosaic_paste (all[bk], by, bx); used[bk] = 1;
    }
